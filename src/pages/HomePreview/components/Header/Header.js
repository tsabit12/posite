...
const CutsomButton = styled(Button)(({ active }) => ({
...
const Header = ({ framework, title, activePage, onChangePage, onDownload }) => {
  ...
  const findIcon = frameworkDefault.find(
    (row) => row.id === framework.frameworkid
  );
  if (findIcon) {
    icon = findIcon.icon;
        ...
        {Object.keys(framework).length > 0 && (
          <Box sx={{ marginTop: "5px" }}>
            <Stack direction={"row"} alignItems="center" spacing={"10px"}>
              <Avatar
                alt={framework.author}
                src={`${process.env.REACT_APP_PUBLIC_URL}${framework.person}`}
              />
          ...
          >
            <Stack direction={"row"} spacing={"10px"} alignItems="center">
              <CutsomButton
                ...
                startIcon={
                  <img
                    src={`${process.env.REACT_APP_PUBLIC_URL}/assets/icon/ant-design_eye-outlined.svg`}
                    alt="eye-open"
                  />
                }
              ...
              <CutsomButton
                ...
                startIcon={
                  <img
                    src={`${process.env.REACT_APP_PUBLIC_URL}/assets/icon/bx_code-alt.svg`}
                    alt="code"
                  />
                }
